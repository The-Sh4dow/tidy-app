...
// MARK: - Menu Bar Label (with today counter)
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        ...
                        if organizer.preferences.silentMode {
            ...
            VStack(spacing: 2) {
                ...
                // Silent mode toggle
                MenuBarButton(
                    icon: organizer.preferences.silentMode ? "bell.fill" : "bell.slash.fill",
                    label: organizer.preferences.silentMode ? "Activar notificaciones" : "Modo silencioso",
                    color: organizer.preferences.silentMode ? .blue : .orange
                ) {
                    organizer.preferences.silentMode.toggle()
                    organizer.saveData()
                }
            }
            ...
            // Stats strip
            HStack(spacing: 0) {
                StatStrip(value: "\(organizer.movedToday)", label: "hoy", icon: "sun.max.fill", color: .yellow)
                Divider().frame(height: 28)
                StatStrip(value: "\(organizer.weeklyStats.totalMoved)", label: "esta semana", icon: "checkmark.circle.fill", color: .green)
                Divider().frame(height: 28)
                StatStrip(value: organizer.weeklyStats.topCategory ?? "—", label: "top categoría", icon: "star.fill", color: .blue)
            }
            .padding(.vertical, 8)
            ...
            if !organizer.moveLog.isEmpty {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    ForEach(organizer.moveLog.prefix(3)) { entry in
                        MenuBarRecentRow(entry: entry) { organizer.undo(entry: entry) }
                    }
                }
            }
...
struct StatStrip: View {
    ...
    var body: some View {
        VStack(spacing: 2) {
            ...
            Text(value).font(.system(size: 13, weight: .bold, design: .rounded)).lineLimit(1).minimumScaleFactor(0.6)
...
struct MenuBarRecentRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(entry.fileName).font(.system(size: 12)).lineLimit(1)
                Text("→ \(entry.category)").font(.system(size: 10)).foregroundColor(.secondary)
            }
            Spacer()
            if !entry.undone && isHovered {
                Button("Deshacer") { onUndo() }.font(.system(size: 10)).buttonStyle(.bordered)
            }
        }
        .padding(.horizontal, 14)
        .padding(.vertical, 5)
